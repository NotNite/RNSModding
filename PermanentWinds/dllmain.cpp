...
template<StringLiteral name> RValue& winds(CInstance* self, CInstance* other, RValue& return_value, int num_args, RValue** args) {
	auto runner = g_yytk_interface->GetRunnerInterface();
	auto arg = RValue(runner.Script_Find_Id("bp_tailwind_permanent"));
	RValue* arg_ptr = &arg;
	auto arg_arr = &arg_ptr;

	orig_funcs[name.value](self, other, return_value, 1, arg_arr);
	
	return_value = orig_funcs[name.value](self, other, return_value, num_args, args);
	return return_value;
}

template<StringLiteral name> RValue& winds2(CInstance* self, CInstance* other, RValue& return_value, int num_args, RValue** args) {
	auto runner = g_yytk_interface->GetRunnerInterface();
	auto arg = RValue(runner.Script_Find_Id("bp_tailwind_permanent"));
	RValue* arg_ptr = &arg;
	auto arg_arr = &arg_ptr;
	...
	return_value = orig_funcs[name.value](self, other, return_value, num_args, args);
	orig_funcs["scr_enemy_add_pattern"](self, other, return_value, 1, arg_arr);
...
void setup_hooks(FWFrame&) {
	...
	g_yytk_interface->PrintInfo("Perma Winds Enabled");
	hook_function("scr_enemy_add_pattern", winds<"scr_enemy_add_pattern">);
	hook_function("gml_Script_scrbp_boss_heal", winds2<"gml_Script_scrbp_boss_heal">);
